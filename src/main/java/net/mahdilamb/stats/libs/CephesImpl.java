...
import static net.mahdilamb.stats.MathUtils.asinh;
import static net.mahdilamb.stats.libs.CephesImpl.beta_c.beta;
import static net.mahdilamb.stats.libs.CephesImpl.beta_c.lbeta;
import static net.mahdilamb.stats.libs.CephesImpl.chbevl_c.chbevl;
import static net.mahdilamb.stats.libs.CephesImpl.ellpe_c.ellpe;
import static net.mahdilamb.stats.libs.CephesImpl.ellpk_c.ellpk;
import static net.mahdilamb.stats.libs.CephesImpl.gamma_c.lgam;
import static net.mahdilamb.stats.libs.CephesImpl.gamma_c.lgam_sgn;
import static net.mahdilamb.stats.libs.CephesImpl.gammasgn_c.gammasgn;
import static net.mahdilamb.stats.libs.CephesImpl.i0_c.i0;
import static net.mahdilamb.stats.libs.CephesImpl.igami_c.igamci;
import static net.mahdilamb.stats.libs.CephesImpl.j0_c.j0;
import static net.mahdilamb.stats.libs.CephesImpl.j0_c.y0;
import static net.mahdilamb.stats.libs.CephesImpl.j1_c.j1;
import static net.mahdilamb.stats.libs.CephesImpl.j1_c.y1;
import static net.mahdilamb.stats.libs.CephesImpl.jv_c.jv;
import static net.mahdilamb.stats.libs.CephesImpl.ndtri_c.ndtri;
import static net.mahdilamb.stats.libs.CephesImpl.polevl_c.p1evl;
import static net.mahdilamb.stats.libs.CephesImpl.polevl_c.polevl;
import static net.mahdilamb.stats.libs.CephesImpl.sinpi_c.sinpi;
import static net.mahdilamb.stats.libs.CephesImpl.unity_c.cosm1;
import static net.mahdilamb.stats.libs.CephesImpl.unity_c.log1pmx;
import static net.mahdilamb.stats.libs.CephesImpl.yn_c.yn;
import static net.mahdilamb.stats.libs.CephesImpl.zeta_c.zeta;
import static net.mahdilamb.stats.libs.Libs.*;
import static net.mahdilamb.stats.libs.SciPy.MACHEP;

/**
 * This "class" contains the actual implementations of Cephes units. The code has been modified from the SciPy
 * repository and the source Cephes library.
 */
/*
...
strictfp final class CephesImpl {
    ...
    static final class igam_c {
        ...
        public static double igamc_series(double a, double x) {
            ...
            logx = log(x);
            term = -unity_c.expm1(a * logx - unity_c.lgam1p(a));
        ...
        public static double asymptotic_series(double a, double x, int func) {
            ...
            res = 0.5 * ndtr_c.erfc(sgn * eta * sqrt(a / 2));
    ...
    static final class unity_c {
        ...
        static final double[] EP = {
                1.2617719307481059087798E-4,
                3.0299440770744196129956E-2,
                9.9999999999999999991025E-1,
        };

        static final double[] EQ = {
    ...
    static final class chdtr_c {
        ...
        static double chdtrc(double df, double x) {
            ...
            return (igam_c.igamc(df / 2.0, x / 2.0));
        }
        ...
        static double chdtr(double df, double x) throws DomainException {
            ...
            return (igam_c.igam(df / 2.0, x / 2.0));
        }
    ...
    static final class igami_c {
        ...
        static double find_inverse_gamma(double a, double p, double q) {
            ...
            if (a == 1) {
                if (q > 0.9) {
                    result = -unity_c.log1p(-p);
                } else {
                    result = -log(q);
                }
            } else if (a < 1) {
                double g = gamma_c.Gamma(a);
                double b = q * g;
                ...
                if ((a >= 500) && (Math.abs(1 - w / a) < 1e-6)) {
                    ...
                    if (w < 0.15 * ap1) {
                        ...
                        z = exp((v + w) / a);
                        s = unity_c.log1p(z / ap1 * (1 + z / ap2));
                        z = exp((v + z - s) / a);
                        s = unity_c.log1p(z / ap1 * (1 + z / ap2));
                        z = exp((v + z - s) / a);
                        s = unity_c.log1p(z / ap1 * (1 + z / ap2 * (1 + z / (a + 3))));
                        z = exp((v + z - s) / a);
                    }
        ...
        static double igami(double a, double p) throws DomainException {
            ...
            /* Halley's method */
            for (i = 0; i < 3; i++) {
                fac = igam_c.igam_fac(a, x);
                if (fac == 0.0) {
                    return x;
                }
                f_fp = (igam_c.igam(a, x) - p) * x / fac;
    ...
    static final class j0_c {
        ...
        static final double DR1 = 5.78318596294678452118E0;

        /* 30.47126234366208639907816317502275584842 */
        static final double DR2 = 3.04712623436620863991E1;

        static final double[] RP = {
    ...
    static final class j1_c {
        ...
        static final double Z1 = 1.46819706421238932572E1;
        static final double Z2 = 4.92184563216946036703E1;
    ...
    static final class ellpk_c {
        ...
        static final double C1 = 1.3862943611198906188E0;    /* log(4) */
    ...
    static final class sindg_c {
        ...
        static final double PI180 = 1.74532925199432957692E-2;    /* pi/180 */
        static final double lossth = 1.0e14;
    ...
    static final class owens_t_c {
        ...
        static double owens_t_norm1(double x) {
            return ndtr_c.erf(x / sqrt(2)) / 2;
        }


        static double owens_t_norm2(double x) {
            return ndtr_c.erfc(x / sqrt(2)) / 2;
        }
    ...
    static final class bdtr_c {
        ...
        static double bdtrc(double k, int n, double p) throws DomainException {
            ...
            if (k == 0) {
                if (p < .01)
                    dk = -unity_c.expm1(dn * unity_c.log1p(-p));
                else
                    dk = 1.0 - pow(1.0 - p, dn);
            } else {
                dk = fk + 1;
        ...
        static double bdtri(double k, int n, double y) throws DomainException {
            ...
            if (fk == 0) {
                if (y > 0.8) {
                    p = -unity_c.expm1(unity_c.log1p(y - 1.0) / dn);
                } else {
                    p = 1.0 - pow(y, 1.0 / dn);
                }
            } else {
    ...
    static final class ellpj_c {
        ...
        static int ellpj(double u, double m, double[] results) {
            ...
            while (Math.abs(c[i] / a[i]) > MACHEP) {
                if (i > 7) {
                    ...
                    return done(twon, a, u, c, i, results, m);
                }
                ai = a[i];
                ++i;
                c[i] = (ai - b) / 2.0;
        ...
        private static int done(double twon, double[] a, double u, double[] c, int i, double[] results, double m) {
            /* backward recurrence */
